...
echo -e "$Building ${bldylw}SlimRemix-ROM ${bldmag}$SLIMREMIX_VERSION_MAJOR${rst} ${bldcya}$SLIMREMIX_VERSION_MINOR ${bldred}$SLIMREMIX_MAINTENANCE${rst}"
...
echo -e ""
echo -e "${bldcya}Looking for SlimRemix product dependencies${rst}${cya}"
if [ "$opt_kr" -ne 0 ]; then
...
if [ "$opt_clean" -eq 1 ]; then
    ...
    . build/envsetup.sh && lunch "slimremix_$device-userdebug";
...
if [ "$opt_fetch" -ne 0 ]; then
    ./vendor/slimremix/tools/cherries.sh $device
fi
